...
import org.apache.mina.common.PooledByteBufferAllocator.PooledByteBuffer;
...
public class MyProtocolCodecFilter extends IoFilterAdapter {
	public static final String ENCODER = MyProtocolCodecFilter.class.getName() + ".encoder";

	public static final String DECODER = MyProtocolCodecFilter.class.getName() + ".decoder";

	private static final String DECODER_OUT = MyProtocolCodecFilter.class.getName() + ".decoderOut";
	...
	public MyProtocolCodecFilter(final ProtocolEncoder encoder, final ProtocolDecoder decoder) {
		if (encoder == null) {
	...
	public MyProtocolCodecFilter(final Class encoderClass, final Class decoderClass) {
	...
	public void onPreAdd(IoFilterChain parent, String name, NextFilter nextFilter) throws Exception {
		if (parent.contains(MyProtocolCodecFilter.class)) {
	...
	public void messageReceived(NextFilter nextFilter, IoSession session, Object message) throws Exception {
		...
		PooledByteBuffer buffer = (PooledByteBuffer)message;
		//System.out.println("buffer.array().length:"+buffer.array().length);
		String smessage = getString(buffer).trim();
		//System.out.println(smessage);
		nextFilter.messageReceived(session, smessage);
	}

	public static String getString(PooledByteBuffer buffer) {
	...
	public void filterWrite(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws Exception {
		...
		try {
			...
			encoderOut.flush();
			nextFilter.filterWrite(session, new WriteRequest(new MessageByteBuffer(writeRequest.getMessage()),
	...
	private static class ProtocolEncoderOutputImpl extends SimpleProtocolEncoderOutput {
		...
		protected WriteFuture doFlush(ByteBuffer buf) {
			...
			nextFilter.filterWrite(session,
					new WriteRequest(new HiddenByteBuffer(buf), future, writeRequest.getDestination()));
			return future;
		}
	}
}
